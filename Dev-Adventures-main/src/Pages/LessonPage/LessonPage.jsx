...
export default function LessonPage() {
    ...
    const checkQuizzes = async () => {
      ...
      for (const lesson of lessons) {
        try {
          ...
          // Store both the status and the quiz data
  ...
  useEffect(() => {
    const checkQuizzes = async () => {
      ...
      // Create an object to store quiz status for each lesson
      const quizStatus = {};
      
      // Check quiz for each lesson
      for (const lesson of lessons) {
        try {
          const response = await axios.get(
            `http://localhost:5101/api/quiz/lesson/${lesson.id}`,
  ...
  const handleOpenQuiz = (lesson, index) => {
    if (lessonQuizzes[lesson.id]) {
      ...
      // Open the quiz creation modal if no quiz exists
